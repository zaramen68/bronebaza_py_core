...
class QueryInstanceType(_StandardInstanceCommand):
    _types = {1: "Push buttons",
              2: "Absolute input device",
              3: "Occupancy sensor",
              4: "Presence sensor"}
    _funit_type = ['InstanceType']
    _inputdev = True
    _response = command.Response
    _opcode = 0x80
...
class QueryInstanceStatusResponse(command.BitmapResponse):
    bits = ["instance error", "instance active"]
    ...
    def error(self):
        ...
        (Ballast not ok)
        """
        return self._value is None or isinstance(self._value, frame.BackwardModuleError) or self.instance_error


class QueryInstanceStatus(_StandardInstanceCommand):
    _inputdev = True
    _response = QueryInstanceStatusResponse
    _opcode = 0x83
