...
from spread_core.protocols.dali import address, command, frame
from spread_core.protocols.dali.compat import python_2_unicode_compatible
...
class QueryStatusResponse(command.BitmapResponse):
    ...
    def error(self):
        ...
        """
        return self._value is None or isinstance(self._value, frame.BackwardModuleError) or self.ballast_status or self.lamp_failure or self.missing_short_address
...
class QueryPhysicalMinimum(_StandardCommand):
    ...
    _cmdval = 0x9a
    _response = command.Response
    _funit_type = ['PhysicalMinLevelRaw']
...
class QueryActualLevel(_StandardCommand):
    ...
    _cmdval = 0xa0
    _response = command.Response
    _funit_type = ['CurrentLevelRaw']


class QueryMaxLevel(_StandardCommand):
    """Return "MAX LEVEL"."""
    _funit_type = ['MaxLevelRaw']
    _cmdval = 0xa1
    _response = command.Response
...
class QuerySystemFailureLevel(_StandardCommand):
    ...
    _cmdval = 0xa4
    _response = command.Response
    _funit_type = ['SystemFailureLevelRaw']
...
class QueryFadeTimeAndRateResponse(command.Response):
    ...
    def value(self):
        super().value
        return dict(FadeRate=self.fade_rate, FadeTime=self.fade_time)
...
class Compare(_SpecialCommand):
    ...
    """
    _cmdval = 0xa9
    _response = command.YesNoErrorResponse
    _force_answer = True
